...
scripts must all import generate_session() from here — never reimplement
session generation independently.
...
class TaskTemplate:
    task_type: TaskType
    # Each step: (tool_name, param_generator_fn). param_generator_fn takes
    # a random.Random instance and returns a params dict, so param variation
...
def generate_session(*, task_type: TaskType, rng: random.Random) -> Session:
    """
    Generate one normal (non-attack) session for the given task type.

    rng is required and keyword-only, no default — callers (calibration,
    evaluation, adversarial) must pass an explicitly seeded Random so
    generation is reproducible and no two scripts silently diverge in
    their randomness assumptions (finding 011).
    """
